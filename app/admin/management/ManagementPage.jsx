...
function FacultyAssignments() {
    ...
    const branchOptions = useMemo(() => Array.from(new Set(data.subjects.map(s => s.branch).filter(Boolean))).sort(), [data.subjects]);
    const semesterOptions = useMemo(() => Array.from(new Set(data.subjects.map(s => s.semester).filter(v => v !== null && v !== undefined && v !== ''))).sort((a, b) => Number(a) - Number(b)), [data.subjects]);
    ...
    return <>
        ...
        <form className={`${styles.panel} ${styles.form}`} onSubmit={submit}>
            ...
            <Select label="Branch" value={form.branch} onChange={setField('branch')} required options={[{ value: '', label: 'Select branch' }, ...branchOptions.map(b => ({ value: b, label: b }))]} />
            <Select label="Semester" value={form.semester} onChange={setField('semester')} required options={[{ value: '', label: 'Select semester' }, ...semesterOptions.map(s => ({ value: s, label: `Semester ${s}` }))]} />
        ...
        <section className={styles.panel} aria-busy={loading}><div className={styles.toolbar}><SearchInput value={query} onChange={setQuery} aria-label="Search faculty assignments" placeholder="Search faculty, subject, or branch" /><Button variant="secondary" onClick={load} loading={loading} iconStart="refresh">Refresh</Button></div>{loading ? <LoadingState label="Loading faculty assignments" /> : visible.length === 0 ? <EmptyState variant="inline" icon="assignment_ind" title="No assignments found" description="Create an assignment to connect faculty and subjects." /> : <>
            <div className={`${styles.tableWrap} gf-desktop-table-wrap`}><table className={styles.table}><thead><tr><th scope="col">Faculty</th><th scope="col">Subject</th><th scope="col">Branch</th><th scope="col">Semester</th><th scope="col">Scheme</th><th scope="col">Class</th><th scope="col">Actions</th></tr></thead><tbody>{visible.map(a => <tr key={a.id}><td>{facultyById.get(a.faculty_id)?.full_name || 'Unknown faculty'}</td><td>{a.subject_code}</td><td>{a.branch}</td><td>{a.semester}</td><td>{a.scheme}</td><td>{classById.get(a.class_id)?.name || 'All classes'}</td><td><div className={styles.actions}><Button size="sm" variant="danger" onClick={() => setConfirming(a)}>Delete</Button></div></td></tr>)}</tbody></table></div>
            <div className="gf-mobile-subject-list">{visible.map(a => <div key={a.id} className="gf-mobile-subject-card">
                <div style={{ fontWeight: 800, fontSize: '14px', color: 'var(--tx-main)' }}>{facultyById.get(a.faculty_id)?.full_name || 'Unknown faculty'}</div>
                <div style={{ fontSize: '12px', color: 'var(--tx-muted)' }}>{a.subject_code}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', background: 'var(--surface-low)', padding: '6px 10px', borderRadius: '6px', fontSize: '12px' }}>
                    <div><span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--tx-dim)', textTransform: 'uppercase' }}>Branch:</span> {a.branch}</div>
                    <div><span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--tx-dim)', textTransform: 'uppercase' }}>Sem:</span> {a.semester}</div>
                    <div><span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--tx-dim)', textTransform: 'uppercase' }}>Scheme:</span> {a.scheme}</div>
                </div>
                <div style={{ fontSize: '12px', color: 'var(--tx-muted)' }}>Class: {classById.get(a.class_id)?.name || 'All classes'}</div>
                <div className={styles.actions}><Button size="sm" variant="danger" onClick={() => setConfirming(a)}>Delete</Button></div>
            </div>)}</div>
        </>}</section>
