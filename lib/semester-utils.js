 ...
 */
export function matchesBranch(studentOrUsn, branchFilter) {
    ...
    const usn = typeof studentOrUsn === 'string' ? studentOrUsn : studentOrUsn?.usn;
    const rawBranch = typeof studentOrUsn === 'object' ? String(studentOrUsn?.branch || '').toUpperCase().trim() : '';
    const usnBranch = extractBranchFromUsn(usn) || '';
