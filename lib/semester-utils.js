 ...
 * so it is a fallback, not an authority. Lateral entry is an attribute of the
 * student and does NOT move them into an earlier cohort: 2AB24CS400 is 24 batch.
 *
...
export function getStudentAcademicBatch(studentOrUsn, isLateral = null, studentYear = null) {
    ...
    const batch = resolveBatch({ usn, year: explicitYear });
    ...
    return {
        ...
        fullYear: batch.year,
        label: batch.label,
        isLateral: isLateralEntry(usn, isLat),
        rawUsnBatch: parsed.valid ? parsed.batchTwoDigit : batch.twoDigit,
        // True when the USN could not supply the batch and the year column had to.
        isOverridden: batch.source !== 'usn',
        source: batch.source
    };
}
...
export function matchesBatch(studentOrUsn, batchFilter, studentYear = null, isLateral = null) {
    ...
    return matchesBatchYear({ usn, year }, batchFilter);
}
