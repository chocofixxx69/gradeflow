...
export function processStudentResults(marks, scheme = '2022', options = {}) {
    const { isLateralEntry = false } = options;
    const grouped = {};
    const stats = {};
...
export function calculateSGPA(subjects, scheme = '2022') {
    ...
    // Normalize total_marks to 0 (not the shared function's null default) when
    // absent, matching this call site's original fallback behavior exactly.
    ...
    return {
        ...
        earnedCredits: earnedCr,  // FIXED: always return the actual numeric value
        backlogs,
