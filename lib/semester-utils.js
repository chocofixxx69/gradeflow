import { unifyGrade, calculateSGPA as calculateSGPACore, isFailedSubject } from './vtuGrades';
...
export function calculateSGPA(subjects, scheme = '2022', branch = null, semester = null) {
    ...
    subjects.forEach(m => {
        ...
        // Use canonical isFailedSubject() — single source of truth
        const isFail = isFailedSubject(m);
        // ── Credit Priority ──────────────────────────────────────────────────────
        // 1. m.credits (DB value from subject_marks, synced from subject_catalog)
        // 2. getOfficialCredit JS catalog (fallback when DB has no credit)
