 ...
 *
 * VTU RULE:
 *   - Regular students:  USN number 001–199 (e.g. 2AB23CS043) → own batch year
 *   - Lateral entry:     USN number ≥ 200   (e.g. 2AB24CS400) → PREVIOUS year’s batch
 *     (Lateral students are admitted in the NEXT calendar year with the NEXT year’s prefix
 *      but belong to the PREVIOUS cohort. e.g. 2AB24CS400 → Batch 2023.)
 *   - 2AB23CS900 has a 900-series number but the SAME year prefix as Batch 2023 regular
 *     students — it stays in Batch 2023 because batch year cannot go below MIN_BATCH_YEAR.
 *
 * @param {string} usn
 * @returns {boolean}
 */
// Earliest batch year at this institution
...
export function isLateralEntryUSN(usn) {
    ...
    const m = u.match(/^[0-9]?[A-Z]{2,3}(\d{2})[A-Z]+(\d{3})/);
    if (!m) return false;
    const yearCode = parseInt(m[1], 10);
    const num = parseInt(m[2], 10);
    if (num < 200) return false; // regular student
    // Lateral only if the resulting cohort year is >= MIN_BATCH_YEAR
    // (prevents 2AB23CS900 from being classified as Batch 2022)
    const cohortYear = 2000 + yearCode - 1;
    return cohortYear >= MIN_BATCH_YEAR;
}
 ...
 * CRITICAL RULES:
 *  1. If an explicit cohort year / batch is set on the student record (e.g. `student.year = 2024` for a year-back student or reassignment),
 *     that explicitly configured batch takes precedence over the USN intake year.
 *  2. Lateral entry students join directly in 2nd year (Semester 3),
 ...
 *  3. Regular students default to their USN intake year.
 *
 * @param {string|object} studentOrUsn - Student object { usn, lateral_entry, year } or USN string
 * @param {boolean|null} isLateral - Optional lateral entry flag if passing USN as string
 * @param {number|string|null} studentYear - Optional student.year if passing USN as string
 */
export function getStudentAcademicBatch(studentOrUsn, isLateral = null, studentYear = null) {
    ...
    const explicitYear = isObject ? (studentOrUsn?.year ?? studentOrUsn?.academic_batch ?? studentOrUsn?.batch) : studentYear;

    if (!usn && !explicitYear) return null;

    const parsed = usn ? extractBatchFromUsn(usn) : null;
    const isLatEntry = isLateralEntry(usn, isLat);

    // If an explicit cohort year is specified (e.g. 2024 for year-back / reassignment)
    if (explicitYear !== null && explicitYear !== undefined && String(explicitYear).trim() !== '') {
        const cleanYrStr = String(explicitYear).replace(/[^0-9]/g, '');
        if (cleanYrStr.length >= 2) {
            const yr2Digit = cleanYrStr.slice(-2);
            const yr4Digit = '20' + yr2Digit;
            const defaultTwoDigit = parsed ? (isLatEntry ? String(parseInt(parsed.twoDigit, 10) - 1).padStart(2, '0') : parsed.twoDigit) : null;
            const isCustom = defaultTwoDigit !== null && defaultTwoDigit !== yr2Digit;

            return {
                twoDigit: yr2Digit,
                fullYear: yr4Digit,
                label: `${yr2Digit} Batch (${yr4Digit})`,
                isLateral: Boolean(isLatEntry),
                rawUsnBatch: parsed?.twoDigit || yr2Digit,
 ...
 * Accurately aligns:
 *  - Year-back and reassigned students (via student.year / explicitYear)
 *  - Lateral entry students (e.g. 2AB24CS401) with their true academic cohort (23 Batch / 2023)
 *  - Regular intake students
 */
export function matchesBatch(studentOrUsn, batchFilter, studentYear = null, isLateral = null) {
    ...
    const usn = isObject ? studentOrUsn?.usn : (typeof studentOrUsn === 'string' ? studentOrUsn : '');
    const isLat = isObject ? (studentOrUsn?.lateral_entry ?? isLateral) : isLateral;
    const yr = isObject ? (studentOrUsn?.year ?? studentOrUsn?.academic_batch ?? studentOrUsn?.batch) : studentYear;

    const cohort = getStudentAcademicBatch(usn, isLat, yr);
    if (cohort) {
        return cohort.twoDigit === filter2Digit || cohort.fullYear === filter4Digit;
    }

    if (yr) {
        const yDigits = String(yr).replace(/[^0-9]/g, '');
        return yDigits === digits || yDigits.slice(-2) === filter2Digit;
    }
