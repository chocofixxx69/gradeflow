...
/**
 * Robust branch matcher that compares both stored branch and USN branch code,
 * supporting aliases across all VTU engineering branches.
 */
export function matchesBranch(studentOrBranchOrUsn, branchFilter) {
    ...
    const filter = String(branchFilter).toUpperCase().trim();
    const str = typeof studentOrBranchOrUsn === 'string' ? studentOrBranchOrUsn.toUpperCase().trim() : '';
    const rawBranch = typeof studentOrBranchOrUsn === 'object' ? String(studentOrBranchOrUsn?.branch || '').toUpperCase().trim() : str;
    const usn = typeof studentOrBranchOrUsn === 'object' ? studentOrBranchOrUsn?.usn : str;
    const usnBranch = (extractBranchFromUsn(usn)) || str;

    const aliasGroups = [
        ['CS', 'CSE', 'COMPUTER'],
        ['CI', 'AI', 'AIML', 'ARTIFICIAL'],
        ['CD', 'DS', 'DATA'],
        ['EC', 'ECE', 'ENC', 'ELECTRONIC'],
        ['EE', 'EEE', 'ELECTRICAL'],
        ['CV', 'CIVIL'],
        ['ME', 'MECH', 'MECHANICAL'],
        ['RI', 'ROBOTICS'],
    ...
    const targetGroup = aliasGroups.find(group => group.some(g => filter.includes(g) || g.includes(filter)));

    if (targetGroup) {
        return targetGroup.some(g => 
            rawBranch.includes(g) || 
            usnBranch === g || 
            usnBranch.includes(g)
        );
    }

    return rawBranch.includes(filter) || usnBranch.includes(filter);
}
