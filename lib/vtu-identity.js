...
/**
 * Dynamic, future-proof batch calculations.
 * Works for ANY year (2022, 2026, 2027, 2030, etc.) without hardcoding.
 */
export function computeBatchRange(year) {
    const y = parseInt(year, 10);
    if (isNaN(y)) return String(year || '');
    const gradYear = y + 4;
    const gradShort = String(gradYear % 100).padStart(2, '0');
    return `${y}–${gradShort}`;
}

export function computeGraduatingClass(year) {
    const y = parseInt(year, 10);
    if (isNaN(y)) return `Class of ${year}`;
    return `Class of ${y + 4}`;
}

export function computeBatchStanding(year, referenceYear = new Date().getFullYear()) {
    const y = parseInt(year, 10);
    if (isNaN(y)) return '';
    const diff = referenceYear - y;
    if (diff <= 0) return '1st Year (Upcoming / Freshers)';
    if (diff === 1) return '2nd Year';
    if (diff === 2) return '3rd Year';
    if (diff === 3) return 'Final Year (4th Year)';
    return 'Graduated / Alumni';
}

export function computeBatchLabel(year, referenceYear = new Date().getFullYear()) {
    const y = parseInt(year, 10);
    if (isNaN(y)) return `Batch ${year}`;
    const range = computeBatchRange(y);
    const standing = computeBatchStanding(y, referenceYear);
    return standing ? `Batch ${range} (${standing})` : `Batch ${range}`;
}

let activeBatchRegistry = [];

export function configureBatchRegistry(rows = []) {
    if (!Array.isArray(rows)) return activeBatchRegistry;
    activeBatchRegistry = rows
        .map(r => ({
            year: String(r.year || '').trim(),
            label: r.label || computeBatchLabel(r.year),
            academic_year: r.academic_year || `${r.year}-${Number(r.year) + 1}`,
            default_scheme: r.default_scheme || (Number(r.year) >= 2025 ? '2025' : '2022'),
            is_active: r.is_active !== false,
            sort_order: Number(r.sort_order ?? r.year ?? 0)
        }))
        .filter(b => b.year);
    return activeBatchRegistry;
}

export function listConfiguredBatches() {
    return activeBatchRegistry;
}
