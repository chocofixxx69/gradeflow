...
export function getSavedFilters(overrides = {}) {
    ...
    try {
        ...
        if (raw) {
            ...
            if (parsed.semester === 3 || parsed.semester === '3') {
                parsed.semester = 'all';
            }
...
export function prewarmAnalyticsPage(href) {
    ...
    if (href.includes('/semester-analysis')) {
        ...
        preloadApi('/api/faculty/analytics/batch-report', {
            branch: filters.branch,
            batch: filters.batch,
            upToSemester: (filters.semester && filters.semester !== 'all') ? filters.semester : 6
        });
        ...
        preloadApi('/api/faculty/analytics/eligibility', {
            branch: filters.branch,
            batch: filters.batch,
            targetSemester: (filters.semester && filters.semester !== 'all') ? filters.semester : 6
        });
