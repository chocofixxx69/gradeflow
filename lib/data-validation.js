...
export function validateDataset({
    ...
    // Canonical CGPA vs what the derived tables would have produced.
    //
    // This is the check that catches a surface drifting back onto academic_remarks
    // or results.total_credits. `records` is the canonical set built from
    // subject_marks + subject_catalog; a large gap here means the published tables
    // are stale for that student and any page reading them shows a wrong CGPA.
    if (studentRecords && studentRecords.size > 0) {
        const divergent = [];
        for (const rec of studentRecords.values()) {
            const perSem = [];
            for (const sem of Object.keys(rec.semStats).map(Number)) {
                const pv = rec.provenance[sem];
                if (pv && pv.sgpaDelta > 0.5) {
                    perSem.push({ semester: sem, computed: rec.semStats[sem].sgpa, published: pv.publishedSgpa, delta: pv.sgpaDelta });
                }
            }
            if (perSem.length === 0) continue;

            // What a flat-20-credit average over academic_remarks would have shown -
            // the formula the student detail page used before it was unified.
            const publishedSgpas = Object.values(rec.provenance)
                .map(p => p.publishedSgpa)
                .filter(v => Number.isFinite(v) && v > 0);
            const staleCgpa = publishedSgpas.length
                ? Number((publishedSgpas.reduce((a, b) => a + b, 0) / publishedSgpas.length).toFixed(2))
                : null;

            divergent.push({
                usn: rec.usn,
                name: rec.name,
                canonicalCgpa: rec.cgpa,
                cgpaIfReadFromRemarks: staleCgpa,
                cgpaGap: staleCgpa !== null ? Number(Math.abs(staleCgpa - rec.cgpa).toFixed(2)) : null,
                staleSemesters: perSem.map(x => `S${x.semester}: marks ${x.computed} vs published ${x.published}`).join('; ')
            });
        }

        if (divergent.length) {
            issues.push(issue({
                code: 'CGPA_SOURCE_DIVERGENCE', severity: SEVERITY.CRITICAL, category: CATEGORIES.ACADEMIC,
                title: 'Published SGPA is stale against the marks it was computed from',
                detail: 'academic_remarks disagrees with subject_marks by more than half a grade point for at least one semester of these students. Every surface now reads the canonical record (lib/student-record.js), so what is shown is correct - but any code that goes back to academic_remarks.sgpa or results.total_credits will show the figure in the "cgpaIfReadFromRemarks" column instead.',
                impact: 'A student whose published rows are stale would be shown a CGPA their own mark sheet contradicts.',
                remedy: 'Re-scrape the listed semesters so academic_remarks matches the marks, and keep reading CGPA from lib/student-record.js.',
                count: divergent.length, total: studentRecords.size, unit: 'students',
                samples: divergent.sort((a, b) => (b.cgpaGap || 0) - (a.cgpaGap || 0))
            }));
        }
    }
