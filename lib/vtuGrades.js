 ...
 *  3. Absent grades             → fail  (A, AB, ABSENT)
 *  4. Explicit passing grades   → pass  (O, S, A+, B+, B, C, P, PP)
 *     ── If grade is set to a passing value we TRUST it and never override
 *        with mark thresholds. This prevents false backlogs for labs / audits
 *        with partial mark data. ──
 *  5. No grade at all           → fall back to marks only (tot < 40 or ext < 18)
 *     and only match exact 'F'/'FAIL' result strings, not substrings.
 *
 * NOTE: 'NP' (Not Passed) is used for non-credit/audit courses. Those are NOT
 * counted as backlogs in SGPA — they're filtered out upstream. Don't add NP here.
 */
export function isFailedSubject(m) {
    ...
    // 2. Explicit failing grades
    const FAIL_GRADES = new Set(['F', 'FAIL', 'NE', 'X']);
    if (FAIL_GRADES.has(grade)) return true;

    // 3. Absent grades
    // NOTE: In VTU raw data, 'A' alone = Absent. The passing grade is 'A+'.
    const ABSENT_GRADES = new Set(['A', 'AB', 'ABSENT']);
    if (ABSENT_GRADES.has(grade)) return true;

    // 4. Explicit passing grades — TRUST the grade, skip mark checks
...
export function getGradeDetails(mark, scheme = '2022') {
    ...
    if (rawGrade === 'F' || rawGrade === 'A' || rawGrade === 'AB' || rawGrade === 'FAIL' || rawGrade === 'ABSENT' || rawGrade === 'NP' || mark?.is_backlog) {
        isFail = true;
        letterGrade = (rawGrade === 'A' || rawGrade === 'AB' || rawGrade === 'ABSENT') ? 'AB' : 'F';
    } else if (total !== null && total !== undefined && total !== '' && !isNaN(Number(total))) {
        const score = Math.round(Number(total));
    ...
    } else if (['O', 'S', 'A+', 'A', 'B+', 'B', 'C', 'P'].includes(rawGrade)) {
