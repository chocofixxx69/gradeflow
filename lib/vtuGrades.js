...
export function getGradeDetails(mark, scheme = '2022') {
    ...
    if (mark?.is_backlog) {
        ...
        isFail = true;
        letterGrade = 'F';
    } else if (['O', 'S', 'A+', 'B+', 'B', 'C', 'P', 'PP', 'D', 'E'].includes(rawGrade)) {
        // Unambiguous passing letter grades
        isFail = false;
...
export function calculateSGPA(subjects, scheme = '2022') {
    ...
    for (const s of (subjects || [])) {
        ...
        // Resolve official credits from VTU curriculum first, then subject credits
        const offCr = getOfficialCredit(s.subject_code || s.code, scheme);
        const cr = offCr !== null ? offCr : (Number(s.credits) || 0);
