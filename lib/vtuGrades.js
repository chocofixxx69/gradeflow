...
export function isFailedSubject(m) {
    ...
    // 1. Explicit backlog flag
    if (m.is_backlog === true || m.is_backlog === 'true') return true;

    const grade = (m.grade || '').trim().toUpperCase();
    const tot = Number(m.total_marks ?? m.total) || 0;
    const ext = Number(m.see_marks ?? m.external) || 0;

    // 2. Unambiguous failing grades
    const FAIL_GRADES = new Set(['F', 'FAIL', 'NE', 'X']);
    if (FAIL_GRADES.has(grade)) return true;

    // 3. Unambiguous passing grades — trust the grade, skip mark checks
    const PASS_GRADES = new Set(['O', 'S', 'A+', 'B+', 'B', 'C', 'P', 'PP', 'D', 'E']);
    if (PASS_GRADES.has(grade)) return false;

    // 4. Ambiguous 'A' / 'AB' / 'ABSENT':
    //    In VTU raw data 'A' alone can mean Absent OR the letter grade A (70-79%).
    //    Disambiguate using total marks: if total >= 40, student passed.
    if (grade === 'A' || grade === 'AB' || grade === 'ABSENT') {
        if (tot >= 40) return false; // letter grade A — student passed
        return true;                 // Absent or genuinely failed
    }

    // 5. No grade — fall back to marks only
    if (tot > 0 && tot < 40) return true;
    if (ext > 0 && ext < 18) return true;

    // Exact result string match only — never substring to avoid 'Fulfilled' false positives
    const resStr = (m.result || m.result_status || '').trim().toUpperCase();
    if (resStr === 'F' || resStr === 'FAIL') return true;

    return false;
}

export function getGradeDetails(mark, scheme = '2022') {
    const total = mark?.total_marks ?? mark?.total;
    const ext = mark?.see_marks ?? mark?.external;
    const rawGrade = (mark?.grade || '').trim().toUpperCase();
    const unified = unifyGrade(rawGrade);
    const score = total !== null && total !== undefined && total !== '' ? Math.round(Number(total)) : null;
    const extScore = ext !== null && ext !== undefined && ext !== '' ? Number(ext) : null;

    let letterGrade = 'F';
    let isFail = false;

    if (mark?.is_backlog) {
        // Explicit backlog flag — always fail
        isFail = true;
        letterGrade = 'F';
    } else if (rawGrade === 'F' || rawGrade === 'FAIL') {
        isFail = true;
        letterGrade = 'F';
    } else if (rawGrade === 'NE' || rawGrade === 'X') {
        isFail = true;
        letterGrade = 'F';
    } else if (rawGrade === 'AB' || rawGrade === 'ABSENT') {
        // Unambiguously absent
        isFail = true;
        letterGrade = 'AB';
    } else if (rawGrade === 'A') {
        // Ambiguous: 'A' = Absent OR letter grade A (70-79%).
        // Disambiguate by total marks: if total >= 40, it is the letter grade A (pass).
        if (score !== null && score >= 40) {
            isFail = false;
            letterGrade = 'A';
        } else {
            isFail = true;
            letterGrade = 'AB'; // treat as Absent
        }
    } else if (rawGrade === 'NP') {
        // Non-credit mandatory course: Not Passed but handled upstream, not a backlog
        isFail = true;
        letterGrade = 'F';
    } else if (['O', 'S', 'A+', 'B+', 'B', 'C', 'D', 'E'].includes(rawGrade)) {
        // Explicit high-distinction letter grades
        isFail = false;
        letterGrade = rawGrade === 'S' ? 'O' : rawGrade;
    } else if (rawGrade === 'P' || rawGrade === 'PP' || rawGrade === 'PASS' || unified === 'P') {
        // Pass status: if numerical score is present, derive exact letter grade (O, A+, A, B+, B, C, P)
        isFail = false;
        if (score !== null && !isNaN(score) && score >= 40) {
            letterGrade = getGradeFromTotal(score, scheme);
        } else {
            letterGrade = 'P';
        }
    } else if (score !== null && !isNaN(score)) {
        // No grade letter — derive from marks
        if (score < 40 || (extScore !== null && extScore > 0 && extScore < 18)) {
            isFail = true;
            letterGrade = 'F';
        } else {
            letterGrade = getGradeFromTotal(score, scheme);
    ...
    const gp = isFail ? 0 : getGradePoint(letterGrade, scheme, total, ext);

    return {
        grade: letterGrade,
        gp,
        gpFormatted: gp.toFixed(2),
        isPass: !isFail && gp > 0,
        isFail
    };
}
...
export function getSubjectInfo(code, scheme = '2022') {
    if (!code) return null;
    const cleanCode = String(code).toUpperCase().trim();
    for (const b in VTU_OFFICIAL_SUBJECT_DATA) {
        if (!b.startsWith(scheme)) continue;
        for (const s in VTU_OFFICIAL_SUBJECT_DATA[b]) {
            const found = VTU_OFFICIAL_SUBJECT_DATA[b][s].find(sub => sub.code.toUpperCase() === cleanCode);
            if (found) return found;
        }
    }
    for (const b in VTU_SUBJECT_DATA) {
        for (const s in VTU_SUBJECT_DATA[b]) {
            const found = VTU_SUBJECT_DATA[b][s].find(sub => sub.code === cleanCode);
            if (found) return found;
        }
    }
    const cr = getOfficialCredit(cleanCode, scheme);
    if (cr !== null) {
        return { code: cleanCode, credits: cr };
    }
    return null;
}

export function getSubjectsFor(branch, sem, scheme = '2022') {
    const cleanBranch = (branch || 'CS').toUpperCase().trim();
    const key = `${scheme}_${cleanBranch}`;
    let list = VTU_OFFICIAL_SUBJECT_DATA[key]?.[String(sem)];
    
    if (!list || list.length === 0) {
        if (sem <= 2) {
            const commonKey = `${scheme}_COMMON`;
            list = VTU_SUBJECT_DATA[commonKey]?.[String(sem)] || VTU_SUBJECT_DATA['2022_COMMON']?.[String(sem)] || [];
        } else {
            list = VTU_SUBJECT_DATA[cleanBranch]?.[String(sem)] || [];
        }
    }
