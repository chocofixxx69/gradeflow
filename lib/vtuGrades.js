...
export function getGradePoint(grade, scheme = '2022', totalMarks = null, externalMarks = null, subjectCode = null) {
    const g = (grade || '').trim().toUpperCase();
    const unified = unifyGrade(g);
    if (['F', 'A', 'X', 'NE', 'ABSENT', 'AB', 'FAIL', 'NP'].includes(unified) || g === 'F' || g === 'A' || g === 'AB' || g === 'NP') return 0;

    // Defense against corrupted pass grade if SEE was failed (< 18) for an SEE course
    if (subjectCode && !isCieOnlyCourse(subjectCode) && externalMarks !== null && externalMarks !== undefined && externalMarks !== '') {
        const ext = Number(externalMarks);
        if (!isNaN(ext) && ext < 18) return 0;
    }
...
/**
 * Authoritative VTU Letter Grade Resolver.
 * Resolves scraped mark records to their authentic letter grade (O, A+, A, B+, B, C, P, F, AB).
 * Eliminates the data issue where VTU scraper stored generic 'P' for all passing marks.
 */
export function resolveCanonicalGrade(mark, scheme = '2022') {
    if (!mark) return '—';
    if (isFailedSubject(mark)) {
        const rawG = String(mark.rawGrade || mark.grade || '').toUpperCase().trim();
        if (['AB', 'ABSENT'].includes(rawG)) return 'AB';
        return 'F';
    }

    const rawG = String(mark.rawGrade || mark.grade || '').toUpperCase().trim();
    if (['O', 'S', 'A+', 'B+', 'B', 'C', 'D', 'E'].includes(rawG)) {
        return rawG === 'S' ? 'O' : rawG;
    }

    const tot = mark.totalMarks ?? mark.total_marks ?? mark.total;
    const totNum = (tot !== null && tot !== undefined && tot !== '' && !isNaN(Number(tot))) ? Number(tot) : null;

    if (rawG === 'A') {
        if (totNum !== null && totNum >= 40) {
            return getGradeFromTotal(totNum, scheme);
        }
        return 'AB';
    }

    if (totNum !== null && totNum > 0) {
        return getGradeFromTotal(totNum, scheme);
    }

    const cie = mark.internalMarks ?? mark.cie_marks ?? mark.internal;
    const see = mark.seeMarks ?? mark.see_marks ?? mark.external;
    ...
    if (cieNum !== null || seeNum !== null) {
        const sum = (cieNum || 0) + (seeNum || 0);
        if (sum > 0) return getGradeFromTotal(sum, scheme);
    }

    return rawG === 'P' || rawG === 'PASS' || rawG === 'PP' ? 'P' : (rawG || 'P');
}
...
export async function calculateSGPA(subjects, scheme = '2022') {
    ...
    for (const s of (subjects || [])) {
        ...
        const subCode = s.subject_code || s.code || s.subjectCode;
        if (isAuditCourse(subCode) || s.isAudit || s.is_audit) continue;

        const gp = getGradePoint(s.grade, scheme, s.total_marks || s.total || null, s.see_marks ?? s.external ?? null, subCode);
        ...
        const offCr = dbCr > 0 ? null : await getOfficialCredit(subCode, scheme);
...
export async function calculateCGPA(semesters, scheme = '2022') {
    let tc = 0, tcp = 0;
    const res = await Promise.all(semesters.map(async sem => {
        const { sgpa, totalCredits, totalCrP } = await calculateSGPA(sem.subjects, scheme);
...
// The full curriculum catalog (lib/vtu-curriculum-catalog.js) is a ~2,350-record,
// ~250KB static lookup — generated from subject_catalog and used here only as a
// last-resort fallback when the DB doesn't have a value. A plain top-of-file
// require() of it would bundle the whole table into every client page that touches
// this module (dashboard, analytics, faculty dashboard, ...), even though only the
// Calculator and Curriculum browser ever actually need it. Loading it through one
// shared dynamic import lets webpack split it into its own chunk, fetched on demand
// and cached for the lifetime of the page instead of shipped on every load.
let _catalogPromise = null;
function loadCatalog() {
    if (!_catalogPromise) _catalogPromise = import('./vtu-curriculum-catalog.js');
    return _catalogPromise;
}

export async function getOfficialCredit(subjectCode, scheme = '2022', branch = null, semester = null) {
    if (!subjectCode) return null;
    try {
        const { getOfficialCredit: resolver } = await loadCatalog();
...
export async function getSubjectsFor(branch, sem, scheme = '2022') {
    try {
        const { VTU_OFFICIAL_SUBJECT_DATA } = await loadCatalog();
        ...
        return (list || []).map(item => ({
            ...item,
            credits: isAuditCourse(item.code) ? 0 : item.credits,
            isAudit: isAuditCourse(item.code),
            grade: defaultGrade
        }));
    } catch {
        return [];
