...
function getLegacyPasswordPepper() {
    // Legacy-only: a committed default pepper is public and lets an attacker
    // reproduce any legacy hash offline, so there is no in-code fallback. When
    // it is unset we skip the legacy check (returning null) rather than throw —
    // this runs inside a password-verify attempt, and a misconfig should read as
    // "that password is wrong", not a 500. Affected users can still reset.
    const value = process.env.LEGACY_STUDENT_PASSWORD_PEPPER;
    if (!value) {
        if (!warnedMissingPepper) {
            warnedMissingPepper = true;
            console.warn('[student-auth] LEGACY_STUDENT_PASSWORD_PEPPER unset — legacy SHA-256 hashes cannot be verified.');
        }
        return null;
    }
    return value;
