...
import { unifyGrade, isFailedSubject } from './vtuGrades';
...
/**
 * Arrear/backlog — delegates to the canonical isFailedSubject (lib/vtuGrades.js)
 * so Result Analysis agrees with every other surface in the app (Student Results,
 * Class Section, admin analytics): trusts the stored is_backlog flag first, then
 * disambiguates letter-grade 'A' (70-79%, a pass) from Absent using marks, rather
 * than treating every raw 'A' grade as an arrear.
 */
export function isArrear(mark) {
    return isFailedSubject(mark);
}
