...
/**
 * Checks if a course is evaluated purely through Continuous Internal Evaluation (CIE)
 * with no Semester End Examination (SEE) prescribed in VTU NEP regulations.
 * e.g. Audit courses, Mini Projects (586), Project Phase I (685),
 * Social Connect (BSCK307), Indian Constitution (1BICO*), Soft Skills (1BSKS*).
 */
export function isCieOnlyCourse(subjectCode) {
    if (!subjectCode) return false;
    const code = String(subjectCode).toUpperCase().trim();
    if (isAuditCourse(code)) return true;
    if (code.endsWith('586') || code.endsWith('685')) return true;
    if (code === 'BSCK307' || code.startsWith('1BICO') || code.startsWith('1BSKS')) return true;
    return false;
}
...
export function normalizeSubjectResult(mark, scheme = '2022', branch = null, semester = null, catalogIndex = null) {
    ...
    // VTU SEE Examination Passing Requirement:
    // For courses with an SEE exam (!isCieOnly), passing minimum is 18/50 (35%).
    // If seeNum is recorded and seeNum < 18, student fails SEE and fails the course unconditionally.
    const failedSee = !isCieOnly && seeNum !== null && seeNum < 18;
    ...
    else if (isBacklogFlag || ['F', 'FAIL', 'NE', 'X'].includes(rawGrade)) {
        // Guard against corrupted/stale backlog flags: If a student genuinely scored total >= 40
        // AND met the SEE requirement (seeNum >= 18, or course is CIE-only).
        // e.g. BMATS201 with total=67, ext=21, int=46 must NEVER be classified as F.
        if (totNum !== null && totNum >= 40 && (isCieOnly || (seeNum !== null && seeNum >= 18))) {
    ...
    // ── 5. Scraped Pass Status ('P', 'PP', 'PASS') ──
        ...
        if (totNum !== null && totNum >= 40 && (isCieOnly || (seeNum !== null && seeNum >= 18))) {
            ...
            isFailed = true;
            isPassed = false;
        } else if (!isCieOnly && seeNum !== null && seeNum < 18) {
            grade = 'F';
            gradePoint = 0;
            isFailed = true;
            isPassed = false;
    ...
    // ── 6. Fallback from Numeric Marks ──
        ...
        if (totNum < 40 || (!isCieOnly && seeNum !== null && seeNum < 18)) {
            grade = 'F';
            gradePoint = 0;
            isFailed = true;
            isPassed = false;
