...
export function normalizeBranch(branchInput, usn = '') {
    if (branchInput && typeof branchInput === 'string') {
        ...
        // EE must be checked before EC: a real branch label on file,
        // "Electrical & Electronics (EEE)", contains the substring "ELECTRONICS" —
        // so when the EC check ran first here, every EEE student was silently
        // misclassified as EC, which then resolved their credits/SGPA against the
        // wrong branch's subject catalog. Confirmed against live data (6 real
        // EEE students affected). Exact-code checks are unambiguous either way;
        // only the substring checks for these two branches can collide, so this
        // reordering is the complete fix.
