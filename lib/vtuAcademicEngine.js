...
export function normalizeSubjectResult(mark, scheme = '2022', branch = null, semester = null) {
    const code = (mark.subject_code || mark.code || '').trim().toUpperCase();
    const name = (mark.subject_name || mark.name || '').trim();
    const rawGrade = (mark.grade || '').trim().toUpperCase();
    const isBacklogFlag = mark.is_backlog === true || mark.is_backlog === 'true';

    const cie = mark.cie_marks ?? mark.internal ?? null;
    const see = mark.see_marks ?? mark.external ?? null;
    const tot = mark.total_marks ?? mark.total ?? null;
    ...
    const credits = isAudit ? 0 : resolveSubjectCredits(mark, scheme, branch, semester);
    ...
    return {
        ...
        isBacklog: isFailed,
        isAudit,
        announcedDate: mark.announced_date || mark.exam_date || null
    };
}
