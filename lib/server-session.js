...
// These secrets sign every staff/student session token. They MUST come from
// the environment — there is no in-code fallback. A committed default value is
// public the moment the repo is, which lets anyone forge a session for any
// user (confirmed: a student token was minted from the old hardcoded default
// with no login). Missing config fails loud at first use rather than silently
// running on a known-public key.
function requireSecret(name) {
    const value = process.env[name];
    if (!value || value.length < 32) {
        throw new Error(
            `[server-session] ${name} is not configured (needs a random value of at least 32 chars). ` +
            `Set it in the environment — sessions cannot be signed or verified without it.`
        );
    }
    return value;
}
...
function getSessionSecret() {
    return requireSecret('SERVER_SESSION_SECRET');
}
