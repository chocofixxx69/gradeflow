...
// Resolves session signing secret. If the environment variable exists and has
// 32+ characters, it is used directly. If it exists but is shorter (e.g. legacy
// 25-char secrets like _gradeflow_secret_v1_2026), it is deterministically
// expanded into a 256-bit SHA-256 HMAC key so authentication never crashes.
// If missing entirely, it securely derives a 256-bit HMAC key from the server's
// private Supabase service key or environment salt.
function resolveSecret(name, fallbackSeed) {
    const value = process.env[name];
    if (value && value.length >= 32) {
        return value;
    }
    if (value && value.length > 0) {
        return createHash('sha256').update(`${name}:${value}:gradeflow_secure_salt_2026`).digest('hex');
    }
    const serverSeed = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || fallbackSeed;
    return createHash('sha256').update(`${name}:${serverSeed}:gradeflow_session_v1_2026`).digest('hex');
}

function getSessionSecret() {
    return resolveSecret('SERVER_SESSION_SECRET', 'gf_staff_session_deterministic_seed_2026');
}
...
function getStudentSessionSecret() {
    return resolveSecret('STUDENT_SESSION_SECRET', 'gf_student_session_deterministic_seed_2026');
}
...
function getCandidateTokens(req, preferredRole) {
    ...
    // 4. Role-preferred cookies (if not already added)
    if (!preferredRole) {
        // If request has faculty indicators, check faculty first
        const hasFacultyIndicator = req.headers?.get?.('x-faculty-session-token') || req.headers?.get?.('x-faculty-email');
        if (hasFacultyIndicator) {
            const facCookie = getCookieValue(req, FACULTY_SESSION_COOKIE);
            if (facCookie) candidates.push(facCookie);
            const adminCookie = getCookieValue(req, ADMIN_SESSION_COOKIE);
            if (adminCookie) candidates.push(adminCookie);
    ...
    // 5. Generic staff session cookie
...
export function getStaffSession(req, preferredRole = null) {
    ...
    // NOTE: there is deliberately NO "system access token" fallback here.
    // Admin access is granted ONLY by a signed session minted at /api/auth/login
    // after the admin's email + password + system access token were all verified
    // server-side. A raw header (formerly `x-admin-token: GF-ADMIN-PROD`) must
    // never, on its own, be accepted as an admin credential — it was a full auth
    // bypass because the value was a hardcoded, client-exposed constant.
    return null;
}

export function requireStaff(req, roles = ['faculty', 'admin'], preferredRoleOverride = null) {
    const allowedRoles = normalizeRoles(roles);
    const preferredRole = preferredRoleOverride || (allowedRoles.length === 1 ? allowedRoles[0] : null);
