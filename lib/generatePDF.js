 ...
 */
function calcSGPA(marks, scheme = '2022') {
    ...
    return {
        sgpa,
        totalCredits,
        earnedCredits,  // FIXED: always return numeric value
        backlogs,
...
export function generateResultPDF({
    ...
    semesters.forEach((sem, idx) => {
        ...
        const body = marks.map(m => [
            ...
            unifyGrade(m.grade) || '—',  // Status instead of raw grade
        ]);

        autoTable(doc, {
            startY: y,
            head: [['Code', 'Subject', 'CR', 'CIE', 'SEE', 'Total', 'Status']],
            body,
            theme: 'grid',
            headStyles: {
            ...
            didParseCell: (data) => {
                if (data.column.index === 6 && data.section === 'body') {
                    ...
                    if (statusText === 'F' || statusText === 'A') {
                        data.cell.styles.textColor = [220, 38, 38];
                    } else if (statusText === 'P') {
                        data.cell.styles.textColor = [22, 163, 74];
                    } else if (statusText === 'W') {
