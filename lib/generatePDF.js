...
const DEFAULT_COLLEGE_ADDRESS = 'Bhatkal, Karnataka';
...
export async function generateResultPDF({
    ...
    // ===== 1. INSTITUTION HEADER (COMPACT & PRESTIGIOUS) =====
    ...
    doc.text(collegeName.toUpperCase(), pageWidth / 2, 9.5, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(214, 232, 230);
    const subtitleParts = [collegeAddress, affiliationLine].filter(Boolean);
    doc.text(subtitleParts.join('  —  '), pageWidth / 2, 16.5, { align: 'center' });
    ...
    doc.line(pageWidth / 2 - 25, y + 1.8, pageWidth / 2 + 25, y + 1.8);
