...
export async function generateResultPDF({
    ...
    // Institutional filing name, e.g. "5th Sem Result - 2AB23CS043.pdf". Callers
    // pass one built by lib/format.js's resultFileName(); the transcript default
    // is kept for older call sites.
    fileName = null,
}) {
    if (!catalogIndex) {
    ...
    autoTable(doc, {
        ...
        body: [
            ...
            // VTU lateral entry is the diploma route — the transcript has to say so,
            // otherwise a record that legitimately starts at semester 3 reads as an
            // incomplete transcript to anyone verifying it.
            ['Entry Mode', isLateralEntry ? 'Lateral Entry (Diploma) - admitted to 3rd semester' : 'Regular (1st semester)',
             'Semesters', isLateralEntry ? '3 to 8 (6 semesters)' : '1 to 8 (8 semesters)'],
        ],
    ...
    const outName = (fileName && String(fileName).trim())
        ? String(fileName).trim().replace(/[\\/:*?"<>|]+/g, '-')
        : `GradeFlow_${safeUsn}_Transcript.pdf`;
    if (typeof window !== 'undefined' && doc.save) {
        doc.save(outName.toLowerCase().endsWith('.pdf') ? outName : `${outName}.pdf`);
    }
