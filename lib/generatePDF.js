...
import { getGradePoint, unifyGrade, getGradeDetails } from './vtuGrades';
...
function calcSGPA(marks, scheme = '2022', branch = null, semNumber = null) {
    ...
    validSubs.forEach(m => {
        ...
        if (credits === 0) return; // Non-credit audit course
...
export function generateResultPDF({
    ...
    semesters.forEach((sem) => {
        ...
        const body = marks.map(m => {
            const details = getGradeDetails(m, scheme);
            const offCr = getOfficialCredit(m.subject_code || m.code, scheme, student?.branch, Number(sem));
            const cr = offCr !== null ? offCr : (m.credits || 0);
            const att = m.attempts || 1;
            const attLabel = att === 1 ? '1st Att.' : att === 2 ? '2nd Att.' : att === 3 ? '3rd Att.' : `${att}th Att.`;
            ...
            return [
                ...
                attLabel,
                m.announced_date || m.exam_date || '—',
            ];
        });

        autoTable(doc, {
            startY: y,
            head: [['Code', 'Subject', 'CR', 'CIE', 'SEE', 'Total', 'Grade', 'GP', 'Result', 'Attempts', 'Announced']],
            body,
            theme: 'grid',
            headStyles: {
            ...
            columnStyles: {
                ...
                10: { cellWidth: 18, halign: 'center', fontSize: 6 },
            },
