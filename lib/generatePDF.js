...
export async function generateResultPDF({
    ...
    const marginX = 16;
    const maxUsableY = 268; // Safe bottom limit before running footer

    // ===== 1. INSTITUTION HEADER (OFFICIAL LETTERHEAD WITH AITM CREST) =====
    // High-resolution AITM crest rendered on clean white surface for maximum clarity and true vibrant colors
    const logoH = 25.5; // mm
    const logoW = logoH * (420 / 480); // ~22.3mm maintaining precise aspect ratio
    const logoX = marginX;
    const logoY = 6;
    try {
        if (AITM_LOGO_BASE64) {
    ...
    // Institutional typography alongside logo
    const headerTextX = logoX + logoW + 5.5;

    // Line 1: College Name (Bold, Rich Brand Primary)
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12.5);
    doc.setTextColor(...BRAND_PRIMARY);
    doc.text((collegeName || DEFAULT_COLLEGE_NAME).toUpperCase(), headerTextX, 12);

    // Line 2: Location
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(85, 90, 90);
    doc.text(collegeAddress || DEFAULT_COLLEGE_ADDRESS, headerTextX, 17);

    // Line 3: Affiliation
    doc.setFontSize(7);
    doc.setTextColor(...TEXT_MUTED);
    doc.text(affiliationLine || DEFAULT_AFFILIATION, headerTextX, 21.5);

    // Line 4: Accreditation & Estd (Refined Crest Gold)
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6.8);
    doc.setTextColor(155, 118, 42);
    doc.text(DEFAULT_ACCREDITATION, headerTextX, 26);

    // Dual Accent Divider Bars (Deep Teal + Crest Gold)
    const dividerY = 33.5;
    doc.setFillColor(...BRAND_PRIMARY);
    doc.rect(marginX, dividerY, pageWidth - (marginX * 2), 1.2, 'F');
    doc.setFillColor(...BRAND_GOLD);
    doc.rect(marginX, dividerY + 1.4, pageWidth - (marginX * 2), 0.4, 'F');
    ...
    semesters.forEach((sem) => {
        ...
        if (y + semTotalBlockHeight > maxUsableY) {
            ...
            doc.setFontSize(6.8);
            doc.setTextColor(...TEXT_MUTED);
            doc.text(`${(collegeName || DEFAULT_COLLEGE_NAME).toUpperCase()}  ·  ACADEMIC TRANSCRIPT  ·  ${usn}  ·  ${studentName.toUpperCase()}`, marginX, 10);
            doc.setDrawColor(215, 210, 205);
            doc.setLineWidth(0.25);
            doc.line(marginX, 12.2, pageWidth - marginX, 12.2);
        }
    ...
    // ===== 8. DYNAMIC PAGE FOOTER =====
    const pageCount = doc.getNumberOfPages();
    for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.setFontSize(6.8);
        doc.setTextColor(150, 145, 140);
        doc.text(`${collegeName} · GradeFlow Academic Intelligence System · Page ${i}/${pageCount}`, pageWidth / 2, 288, { align: 'center' });
        doc.text('This is a system-generated report. Data integrity is maintained by the institutional database.', pageWidth / 2, 292, { align: 'center' });
    }
