...
export async function apiRequest(path, { query, responseType = 'json', signal, cacheTtl = 0, ...options } = {}) {
    ...
    // Auto-cache metadata and static lookups for 30 seconds unless explicitly overridden
    const defaultTtl = (path.includes('/api/system/meta') || path.includes('/api/curriculum')) ? 30_000 : cacheTtl;
