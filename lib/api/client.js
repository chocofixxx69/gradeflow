...
function getClientAuthHeaders(url = '') {
    if (typeof window === 'undefined') return {};
    const headers = {};

    const urlStr = String(url || '');
    const isFacultyApi = urlStr.includes('/api/faculty');
    const isAdminApi = urlStr.includes('/api/admin');
    const pathname = window.location.pathname || '';
    const isFacultyPortal = pathname.startsWith('/faculty');
    const isAdminPortal = pathname.startsWith('/admin');

    const facStr = localStorage.getItem('faculty_session');
    const admStr = localStorage.getItem('admin_session');

    // 1. If targeting a faculty endpoint or currently in faculty portal: prioritize faculty auth
    if (isFacultyApi || (isFacultyPortal && !isAdminApi) || (facStr && !isAdminPortal && !isAdminApi)) {
        if (facStr) {
            try {
                ...
                if (fac?.email) headers['x-faculty-email'] = fac.email;
                if (fac?.id) headers['x-faculty-id'] = fac.id;
            } catch {}
        }
        // Do not attach admin tokens to faculty requests
        if (isFacultyApi || isFacultyPortal) {
            return headers;
        }
    }

    // 2. Admin auth
    if (admStr && (isAdminApi || isAdminPortal || !facStr)) {
        try {
    ...
    // 3. Fallback to faculty if staff token still not set
    if (!headers['x-staff-token'] && facStr) {
        try {
            const fac = JSON.parse(facStr);
            if (fac?.sessionToken) {
                headers['x-faculty-session-token'] = fac.sessionToken;
...
async function executeRequest(url, { responseType = 'json', signal, ...options } = {}) {
    const authHeaders = getClientAuthHeaders(url);
    const contentTypeHeader = (options.body && typeof options.body === 'string')
