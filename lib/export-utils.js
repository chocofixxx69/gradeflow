...
export function exportConsolidatedReportPDF({
    ...
    try {
        ...
        const semBatchStr = cleanStr(institutionInfo.batch || `Sem ${selectedClass?.semester || 4} - ${selectedClass?.name || 'Class'}`);
        const ayStr = cleanStr(institutionInfo.ay || 'AY -2025-26 (EVEN Semester)');
        const titleStr = cleanStr(`Result Analysis - Sem ${selectedClass?.semester || 4} - University Exam`);
        ...
        const subList = (subjects && subjects.length > 0)
            ? subjects
            : Array.from(new Set(allMarks.map(m => m.subject_code))).map(code => ({ code, name: code }));
