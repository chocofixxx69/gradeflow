...
export function exportConsolidatedReportCSV({
    ...
    csv += `RESULT ANALYSIS SUBJECT WISE\n`;
    csv += `S.No,Subject Code,Subject Name,Faculty Name,Appeared,Passed,Failed,Pass %\n`;
    subList.forEach((sub, idx) => {
        ...
        csv += `${idx + 1},${cleanStr(sub.code)},"${cleanStr(sub.name || sub.code).replace(/"/g, '""')}","${facName.replace(/"/g, '""')}",${app},${pass},${fail},${passPct}%\n`;
    });
    csv += `\n`;
