// No static `import * as XLSX` here: the only Excel writer left in this module goes
// through lib/workbook-export.js, which loads SheetJS lazily. Keeping xlsx out of
// the module graph also keeps this file importable from a server render, where
// SheetJS touches `self` at evaluation time.
...
import { downloadCSV, downloadWorkbook, safeFileName } from './workbook-export.js';
...
export function exportToExcel(data, fileName) {
    ...
    // lib/workbook-export.js writes with an explicit bookType and the real OOXML
    // MIME type — XLSX.writeFile() infers both and is how "Export Excel" ended up
    // opening in Excel's text importer as mojibake.
    return downloadWorkbook(
        [{ name: 'Results', headers, rows: rows.map(r => headers.map(h => r[h])) }],
        fileName
    );
}
...
export function exportToCSV(data, fileName) {
    const rows = Array.isArray(data) ? data : [];
    const headers = rows.length ? Object.keys(rows[0]) : [];
    downloadCSV([headers, ...rows.map(r => headers.map(h => r[h]))], fileName);
}
...
export function exportClassReportCSV({ selectedClass, students, allMarks = [], subjects = [], subjectToppers, fileName }) {
    ...
    // Rows, not a hand-concatenated string: every field is escaped once, by
    // lib/workbook-export.js, and the file is written with a UTF-8 BOM and CRLF
    // endings so Excel opens it into columns on a double-click instead of
    // decoding it through the machine's ANSI codepage.
    const rows = [];
    rows.push([`CLASS PERFORMANCE REPORT: ${cleanName}`]);
    rows.push(['Branch', selectedClass?.branch || '', 'Semester', selectedClass?.semester || '', 'Scheme', selectedClass?.scheme || '']);
    rows.push(['Total Students', students?.length || 0, 'Generated Date', new Date().toLocaleDateString()]);
    rows.push([]);

    // 1. Overall CGPA Toppers
    rows.push(['OVERALL CLASS TOPPERS (CGPA)']);
    rows.push(['Rank', 'USN', 'Name', 'CGPA', 'Backlogs']);
    const toppers = [...(students || [])].filter(s => s.has_data && s.cgpa != null).sort((a, b) => b.cgpa - a.cgpa).slice(0, 10);
    if (toppers.length > 0) {
        toppers.forEach((st, idx) => {
            rows.push([idx + 1, st.usn, st.name || '', fmtNum(st.cgpa), st.total_backlogs || 0]);
        });
    } else {
        rows.push(['No result data available for toppers']);
    }
    rows.push([]);

    // 2. Class Toppers by SGPA — prefer each student's pre-computed canonical
    // per-semester SGPA/credits (lib/vtuAcademicEngine.js via /api/class-students'
    // semester_data — the same source every other page uses) over recomputing
    // here from raw marks with a credit resolver that can trust a stale
    // subject_marks.credits value instead of the live subject_catalog.
    const marksByUsn = {};
    (allMarks || []).forEach(m => {
        if (!marksByUsn[m.usn]) marksByUsn[m.usn] = [];
        marksByUsn[m.usn].push(m);
    });
    const inferredSem = allMarks?.[0]?.semester ?? selectedClass?.semester ?? null;

    const studentsWithComputed = (students || []).map(s => {
        const uMarks = marksByUsn[s.usn] || [];
        let totalScore = 0;
        uMarks.forEach(m => { totalScore += Number(m.total) || 0; });

        const canonical = inferredSem != null ? s.semester_data?.[inferredSem] : null;
        let sgpa, totalCr;
        if (canonical) {
            sgpa = canonical.sgpa || 0;
            totalCr = canonical.total_credits || 0;
        } else {
            let earnedPoints = 0;
            totalCr = 0;
            uMarks.forEach(m => {
            ...
            sgpa = totalCr > 0 ? Number((earnedPoints / totalCr).toFixed(2)) : 0;
        }
    ...
    if (sgpaToppers.length > 0) {
        rows.push(['CLASS TOPPERS (SGPA WISE)']);
        rows.push(['Rank', 'USN', 'Name', 'SGPA', 'Credits Earned']);
        sgpaToppers.forEach((st, idx) => {
            rows.push([idx + 1, st.usn, st.name || '', fmtNum(st.sgpa), st.totalCr || 0]);
        });
        rows.push([]);
    }
    ...
    if (marksToppers.length > 0) {
        rows.push(['CLASS TOPPERS (TOTAL MARKS WISE)']);
        rows.push(['Rank', 'USN', 'Name', 'Total Marks']);
        marksToppers.forEach((st, idx) => {
            rows.push([idx + 1, st.usn, st.name || '', st.totalScore]);
        });
        rows.push([]);
    }
    ...
    if (subjectToppers && subjectToppers.length > 0) {
        rows.push(['SUBJECT-WISE TOPPERS']);
        rows.push(['Subject Code', 'Subject Name', 'Top Student Name', 'USN', 'Highest Marks']);
        subjectToppers.forEach(st => {
            const topStudent = st.allScores?.[0];
            if (topStudent) {
                rows.push([st.code, st.name || '', topStudent.name || '', topStudent.usn, topStudent.total ?? '']);
            }
        });
        rows.push([]);
    }
    ...
    // 5. Full Roster
    rows.push(['FULL STUDENT ROSTER']);
    rows.push(['#', 'USN', 'Name', 'Entry', 'Semester', 'CGPA', 'SGPA', 'Backlog Status']);
    (students || []).forEach((s, idx) => {
        ...
        rows.push([
            ...
            s.usn,
            s.name || '',
            (s.lateral_entry || s.isLateral) ? 'Lateral (Diploma)' : 'Regular',
            s.semester || '',
            s.has_data && s.cgpa != null ? fmtNum(s.cgpa) : '',
            stComputed && stComputed.sgpa > 0 ? fmtNum(stComputed.sgpa) : '',
            backlogText
        ]);
    });

    downloadCSV(rows, fileName || `${cleanName.replace(/\s+/g, '_')}_Class_Report.csv`);
}
...
export function exportConsolidatedReportCSV({
    ...
    // Rows, not a hand-concatenated string — see exportClassReportCSV above for
    // why (escaping, UTF-8 BOM and CRLF so Excel opens it into columns directly).
    const rows = [];
    rows.push([`INSTITUTIONAL CONSOLIDATED RESULT ANALYSIS: ${deptName}`]);
    rows.push(['Class', cleanName, 'Semester', currentSem, 'Academic Year', institutionInfo.academicYear || '2025-2026']);
    ...
    rows.push(['CLASS PASSING PERCENTAGE SUMMARY']);
    rows.push(['No. of Students Appeared', 'No. of Students Passed', 'No. of Students Failed', 'Pass Percentage']);
    rows.push([totalAppeared, totalPassed, totalFailed, `${passPct}%`]);
    rows.push([]);

    // 2. Result Analysis Subject Wise
    rows.push(['RESULT ANALYSIS SUBJECT WISE']);
    rows.push(['S.No', 'Subject Code', 'Subject Name', 'Faculty Name', 'Appeared', 'Passed', 'Failed', 'Pass %']);
    subList.forEach((sub, idx) => {
        const code = sub.code;
        const cleanSubCode = (code || '').toUpperCase().trim();
        const rawFacName = facultyMap[code]
            || facultyMap[cleanSubCode]
            || (sub.id && facultyMap[sub.id])
            || Object.entries(facultyMap || {}).find(([k]) => (k || '').toUpperCase().trim() === cleanSubCode)?.[1];
        ...
        const passPct = app > 0 ? ((pass / app) * 100).toFixed(2) : '0.00';
        rows.push([idx + 1, cleanStr(sub.code), cleanStr(sub.name || sub.code), facName, app, pass, fail, `${passPct}%`]);
    });
    rows.push([]);

    // 3. Class Toppers (Top 10) — Marks Wise
    rows.push(['CLASS TOPPERS (TOP 10 - MARKS WISE)']);
    rows.push(['Rank', 'USN', 'Name', 'Total Marks']);
    ...
    // 4. Class Toppers (Top 10) — SGPA Wise
    rows.push(['CLASS TOPPERS (TOP 10 - SGPA WISE)']);
    rows.push(['Rank', 'USN', 'Name', 'SGPA', 'Credits Earned']);
    ...
    top10SgpaList.forEach((st, idx) => {
        rows.push([idx + 1, st.usn, st.name || '', fmtNum(st.sgpa), st.totalCredits || 0]);
    });
    rows.push([]);

    // 5. Arrears (Backlog) Analysis
    rows.push(['ARREARS (BACKLOG) ANALYSIS']);
    rows.push(['Rank', 'USN', 'Name', 'No of Arrears']);
    ...
    arrearsList.forEach((st, idx) => {
        rows.push([idx + 1, st.usn, st.name || '', st.arrearsCount]);
    });
    rows.push([]);

    // 6. Complete Marks Matrix
    rows.push([`COMPLETE MARKS MATRIX (SEMESTER ${currentSem})`]);
    rows.push(['Sl. no', 'USN', 'NAME', ...subList.map(sb => `${sb.code} - ${sb.name || ''}`)]);
    studentStats.forEach((s, idx) => {
        ...
        rows.push([
            ...
            s.name || '',
            ...subList.map(sb => {
                const sm = uMarks[sb.code];
                return sm ? sm.total : '';
            })
        ]);
    });

    downloadCSV(rows, fileName || `${cleanName.replace(/\s+/g, '_')}_Sem${currentSem}_Consolidated_Report.csv`);
}
...
export function exportConsolidatedReportPDF({
    ...
    try {
        ...
        const studentStats = (students || []).map(s => {
            ...
            // Prefer each student's pre-computed canonical per-semester SGPA/
            // credits (lib/vtuAcademicEngine.js via /api/class-students'
            // semester_data — the same source every other page uses) over
            // recomputing here from raw marks with a credit resolver that can
            // trust a stale subject_marks.credits value instead of the live
            // subject_catalog.
            const canonical = s.semester_data?.[currentSem];
            let sgpa = 0;
            let totalCredits = 0;
            if (canonical) {
                sgpa = canonical.sgpa || 0;
                totalCredits = canonical.total_credits || 0;
            } else {
                let earnedPoints = 0;
                subList.forEach(sub => {
                    const sm = uMarks[sub.code];
                    if (!sm || isFailedSubject(sm)) return;
                    const cr = resolveSubjectCredits(sm, sub);
                    const gp = scoreToGradePoint(sm.total, sm.grade);
                    earnedPoints += (gp * cr);
                    totalCredits += cr;
                });
                sgpa = totalCredits > 0 ? Number((earnedPoints / totalCredits).toFixed(2)) : 0;
            }
            return { ...s, totalScore, arrearsCount, hasAnyResult, sgpa, totalCredits };
        });
...
export function exportLeaderboardCSV({
    ...
    const rows = [];
    rows.push([`CLASS LEADERBOARD & TOPPERS REPORT: ${cohortName} (${batchCode})`]);
    rows.push(['Total Students', totalStudents, 'Regular', regularCount, 'Lateral Entry (Diploma)', lateralCount, 'Target Semester', targetSemester]);
    rows.push(['Generated Date', new Date().toLocaleDateString()]);
    rows.push([]);

    // 1. Overall CGPA Leaderboard
    rows.push(['OVERALL CLASS LEADERBOARD (CGPA WISE)']);
    rows.push(['Rank', 'USN', 'Name', 'Type', 'CGPA', 'Semesters Tracked', 'Backlogs']);
    (overallLeaderboard || []).forEach(st => {
        rows.push([st.rank, st.usn, st.name || '', st.isLateral ? 'Lateral (Diploma)' : 'Regular', fmtNum(st.cgpa), st.semestersTracked || 0, st.totalBacklogs || 0]);
    });
    rows.push([]);

    // 2. Semester SGPA Leaderboard
    rows.push([`SEMESTER ${targetSemester} LEADERBOARD (SGPA WISE)`]);
    rows.push(['Rank', 'USN', 'Name', 'Type', 'SGPA', 'Credits', 'Status']);
    (semesterLeaderboard || []).forEach(st => {
        rows.push([st.rank, st.usn, st.name || '', st.isLateral ? 'Lateral (Diploma)' : 'Regular', fmtNum(st.sgpa), st.credits || 0, st.statusText || '']);
    });
    rows.push([]);

    // 3. Subject-wise Leaderboard
    if (subjectLeaderboard && subjectLeaderboard.length > 0) {
        rows.push([`SUBJECT LEADERBOARD: ${currentSubject?.subject_code || ''} - ${currentSubject?.subject_name || ''}`]);
        rows.push(['Rank', 'USN', 'Name', 'Type', 'CIE /50', 'SEE /50', 'Total /100', 'Grade']);
        subjectLeaderboard.forEach(st => {
            rows.push([st.rank, st.usn, st.name || '', st.isLateral ? 'Lateral (Diploma)' : 'Regular', st.internal, st.external, st.total, st.grade]);
        });
    }

    downloadCSV(rows, fileName || `${batchCode}_Leaderboard_Report.csv`);
}
