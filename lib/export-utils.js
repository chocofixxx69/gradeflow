...
export function exportConsolidatedReportPDF({
    ...
    try {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...BRAND_PRIMARY);
        doc.text(`Subject Pass Average: ${avgSubjectPass}%`, chartCardX + 6.0, chartCardY + 4.5);
        doc.setTextColor(...TEXT_MUTED);
        doc.setFont('helvetica', 'normal');
        doc.text(`Highest Subject: ${highestSub?.code || '—'} (${highestSub ? highestSub.passPct.toFixed(1) : 0}%)  ·  Evaluated: ${subjectStats.length} Subjects`, chartCardX + 58.0, chartCardY + 4.5);

        // Inner Plotting Area
        const plotX = chartCardX + 16.0;
        const plotY = chartCardY + 7.5;
        ...
        const barW = Math.min(18.0, Math.max(8.0, slotW * 0.55));
        ...
        subjectStats.forEach((s, i) => {
            ...
            doc.rect(xBar, yBar, barW, barH, 'F');

            // Gold Cap on top of bar
            doc.setFillColor(...BRAND_GOLD);
            doc.rect(xBar, yBar, barW, 0.7, 'F');

            // Pass % badge ABOVE the bar
            doc.setFontSize(6.5);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(...barFill);
            doc.text(`${s.passPct.toFixed(1)}%`, xCenter, yBar - 1.0, { align: 'center' });

            // Multi-tier clean labels strictly BELOW the plot axis
            const labelStartY = plotY + plotH + 3.5;

            // Tier 1: Subject Code (Bold, Prominent, Centered)
            doc.setFontSize(6.5);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(...BRAND_PRIMARY);
            doc.text(cleanStr(s.code), xCenter, labelStartY, { align: 'center' });

            // Tier 2: Subject Name (Readable, Centered, Truncated)
            doc.setFontSize(5.8);
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(...TEXT_MAIN);
            const cleanSubName = cleanStr(s.name);
            const truncatedSub = cleanSubName.length > 15 ? cleanSubName.substring(0, 14) + '…' : cleanSubName;
            doc.text(truncatedSub, xCenter, labelStartY + 3.2, { align: 'center' });

            // Tier 3: Faculty Name (Italic, Subtle, Centered)
            doc.setFontSize(5.2);
            doc.setFont('helvetica', 'italic');
            doc.setTextColor(...TEXT_MUTED);
            const cleanFac = s.facName && s.facName !== '—' ? cleanStr(s.facName) : 'Unassigned';
            const truncatedFac = cleanFac.length > 14 ? cleanFac.substring(0, 13) + '…' : cleanFac;
            doc.text(truncatedFac, xCenter, labelStartY + 6.0, { align: 'center' });
        });

        currentY = chartCardY + chartCardH + 6.0;
