...
export function exportClassReportPDF({ selectedClass, students, subjectToppers, fileName }) {
    ...
    doc.text(`Class: ${cleanName}  |  Branch: ${selectedClass?.branch || '—'}  |  Sem: ${selectedClass?.semester || '—'}  |  Scheme: ${selectedClass?.scheme || '—'}  |  Students: ${students?.length || 0}`, 14, 25);
    doc.setFontSize(8);
