...
export const ACTION_REGISTRY = {
    ...
    CLASS_EDIT: {
        title: 'Academic Class Container Modification',
        category: 'CLASS_ROSTER',
        severity: 'ELEVATED',
        defaultModule: 'Faculty Portal > Classes & Structure',
        defaultReason: 'Adjustment of academic class parameters (name, section, faculty, academic year, semester).',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Modified class section properties and academic curriculum configuration.',
    },
    CLASS_UPDATE: {
        title: 'Academic Class Container Modification',
        category: 'CLASS_ROSTER',
    ...
    CLASS_STUDENTS_TRANSFER: {
        title: 'Class Cohort Student Transfer',
        category: 'CLASS_ROSTER',
        severity: 'ELEVATED',
        defaultModule: 'Faculty Portal > Classes > Section Roster',
        defaultReason: 'Reallocation or section transfer of students between class cohorts.',
        defaultMethod: 'Batch Transfer Pipeline',
        description: 'Transferred student roster between academic class sections.',
    },
    ASSIGN_SUBJECT: {
        title: 'Faculty Subject Teaching Load Assignment',
        category: 'CLASS_ROSTER',
        severity: 'INFO',
        defaultModule: 'Faculty Portal > Teaching Load > Subject Mapping',
        defaultReason: 'Curricular course allocation and syllabus subject teaching load assignment for academic semester.',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Allocated subject code and curricular course load to faculty member roster.',
    },
    UNASSIGN_SUBJECT: {
        title: 'Faculty Subject Teaching Load De-allocation',
        category: 'CLASS_ROSTER',
        severity: 'ELEVATED',
        defaultModule: 'Faculty Portal > Teaching Load > Subject Mapping',
        defaultReason: 'Curricular reallocation or semester completion subject removal from faculty teaching roster.',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Removed subject assignment from faculty teaching load roster.',
    },
    ...
    FACULTY_LOGOUT: {
        title: 'Faculty Session Termination / Logout (Check-out)',
        category: 'SECURITY_ACCESS',
        severity: 'INFO',
        ...
        description: 'Instructor safely terminated portal session and checked out.',
    },
    FACULTY_CHECKOUT: {
        title: 'Faculty Session Termination / Logout (Check-out)',
        category: 'SECURITY_ACCESS',
    ...
    FACULTY_CHECKIN: {
        title: 'Faculty Authentication & Portal Session Login (Check-in)',
        category: 'SECURITY_ACCESS',
        severity: 'INFO',
        defaultModule: 'Faculty Portal > Authentication Gateway',
        defaultReason: 'Authorized institutional portal authentication session established.',
        defaultMethod: 'Secure Session Login (HTTPS / Bcrypt)',
        description: 'Instructor authenticated into GradeFlow Faculty Portal with verified credentials.',
    },
    FACULTY_OFFLINE: {
        title: 'Faculty Disconnected / Session Went Offline',
        category: 'SECURITY_ACCESS',
        severity: 'INFO',
        defaultModule: 'Faculty Portal > Session Management',
        defaultReason: 'Instructor tab closed, window navigated away, or session disconnected.',
        defaultMethod: 'Browser Beacon / Disconnect Tracker',
        description: 'Faculty session disconnected and marked offline.',
    },
    FACULTY_HEARTBEAT: {
        title: 'Faculty Live Presence Heartbeat',
        category: 'SECURITY_ACCESS',
        severity: 'INFO',
        defaultModule: 'Faculty Portal > Real-Time Presence',
        defaultReason: 'Active browser session verified via real-time presence heartbeat.',
        defaultMethod: 'Background Ping (20s interval)',
        description: 'Verified instructor active browser session.',
    },
    ...
    STUDENT_STATUS_TOGGLED: {
        title: 'Student Status Adjustment (Active/Inactive)',
        category: 'SECURITY_ACCESS',
        severity: 'ELEVATED',
        defaultModule: 'Faculty Portal > Student Directory',
        defaultReason: 'Modification of student active standing or institutional enrollment hold.',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Updated student enrollment standing (active / inactive / suspended).',
    },
    STUDENT_SUSPENDED: {
    ...
    MARKS_EDIT: {
        title: 'Internal Assessment Marks Modification',
        category: 'MARKS_CIE',
        severity: 'ELEVATED',
        defaultModule: 'Faculty Portal > Internal Evaluation > Marks Entry',
        defaultReason: 'Continuous Internal Evaluation (CIE) score revision following student review.',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Updated recorded internal assessment marks for student subject.',
    },
    MARKS_DELETE: {
        title: 'Student Marks Record Deletion',
        category: 'MARKS_CIE',
        severity: 'CRITICAL',
        defaultModule: 'Faculty Portal > Internal Evaluation',
        defaultReason: 'Purge of duplicate or erroneous marks entry from institutional records.',
        defaultMethod: 'Interactive Web UI (Next.js)',
        description: 'Permanently deleted student subject assessment marks entry.',
    },
    DOWNLOAD_TRANSCRIPT: {
        title: 'Official Academic Transcript Generation',
        category: 'ACADEMIC_AUDIT',
        severity: 'INFO',
        defaultModule: 'Faculty Portal > Student Directory > Transcript Engine',
        defaultReason: 'Issuance of official student academic marks card and CGPA progress transcript.',
        defaultMethod: 'Client Document Pipeline (jsPDF)',
        description: 'Generated printable official academic transcript PDF for student.',
    },
};
...
export function enrichActivityRecord(log, facultyMap = {}, studentMap = {}) {
    ...
    let actionKey = log.action_type || 'VIEW_RECORD';
    if (typeof actionKey === 'string' && (actionKey.startsWith('{') || actionKey.includes('sessionToken') || actionKey.length > 60)) {
        try {
            const parsed = JSON.parse(actionKey);
            actionKey = parsed.action || parsed.action_type || 'CLASS_CREATE';
        } catch {
            actionKey = 'CLASS_CREATE';
        }
    }

    const registryEntry = ACTION_REGISTRY[actionKey] || {
        title: (typeof actionKey === 'string' ? actionKey : 'Academic Operation').replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase()),
        category: 'ACADEMIC_AUDIT',
        severity: 'INFO',
    ...
    // Exact action synthesis for high-clarity UX
    const meta = log.metadata || {};
    let details = log.details;
    if (!details || details.startsWith('Administrator recorded action') || details === registryEntry.description) {
        if (actionKey === 'UNASSIGN_SUBJECT') {
            details = meta.subject_code ? `Removed subject ${meta.subject_code} assignment from ${facultyName}` : `Removed subject assignment from ${facultyName}`;
        } else if (actionKey === 'ASSIGN_SUBJECT') {
            details = meta.subject_code ? `Assigned subject ${meta.subject_code}${meta.branch ? ` (${meta.branch})` : ''}${meta.semester ? ` Sem ${meta.semester}` : ''} to ${facultyName}` : `Assigned subject load to ${facultyName}`;
        } else if (actionKey === 'ASSIGN_SUBJECT_BATCH') {
            const list = Array.isArray(meta.subject_codes) ? meta.subject_codes.join(', ') : '';
            details = `Batch assigned subjects (${list}) to ${facultyName}`;
        } else if (actionKey === 'CLASS_EDIT' || actionKey === 'CLASS_UPDATE') {
            const changed = meta.updates ? Object.keys(meta.updates).join(', ') : '';
            details = `Updated class ${meta.class_name || (log.target_usn ? `(${log.target_usn})` : '')}${changed ? ` (modified: ${changed})` : ''}`;
        } else if (actionKey === 'CLASS_CREATE') {
            details = `Created new class "${meta.name || log.target_usn || ''}" (${meta.branch || ''} Sem ${meta.semester || ''})`;
        } else if (actionKey === 'CLASS_DELETE') {
            details = `Permanently deleted class "${meta.class_name || log.target_usn || ''}"`;
        } else if (actionKey === 'CLASS_ADD_STUDENT') {
            details = `Enrolled student ${log.target_usn || ''} into class roster`;
        } else if (actionKey === 'CLASS_REMOVE_STUDENT') {
            details = `Removed student ${log.target_usn || ''} from class roster`;
        } else if (actionKey === 'CLASS_BULK_IMPORT') {
            details = `Bulk imported ${meta.count || 'cohort'} students into class roster`;
        } else if (actionKey === 'CLASS_STUDENTS_TRANSFER') {
            details = `Transferred ${meta.count || ''} student(s) (${meta.mode || 'move'}) between class cohorts`;
        } else if (actionKey === 'FACULTY_LOGIN' || actionKey === 'FACULTY_CHECKIN') {
            details = `${facultyName} logged in and established authenticated portal session`;
        } else if (actionKey === 'FACULTY_LOGOUT' || actionKey === 'FACULTY_CHECKOUT') {
            details = `${facultyName} ended active session and logged out (check-out)`;
        } else if (actionKey === 'FACULTY_OFFLINE') {
            details = `${facultyName} disconnected or closed browser session (went offline)`;
        } else if (actionKey === 'FACULTY_HEARTBEAT') {
            details = `${facultyName} active in portal (real-time heartbeat ping)`;
        } else if (actionKey === 'STUDENT_PROFILE_UPDATED') {
            details = `Updated academic profile records for ${log.target_usn || ''}`;
        } else if (actionKey === 'STUDENT_STATUS_TOGGLED') {
            details = `Adjusted account active standing for student ${log.target_usn || ''}`;
        } else if (actionKey === 'HALL_TICKET_GENERATE') {
            details = `Generated official VTU SEE Hall Ticket for ${log.target_usn || ''}`;
        } else if (actionKey === 'HALL_TICKET_BATCH_EXPORT') {
            details = `Exported batch examination hall tickets for cohort`;
        } else {
            details = log.details || registryEntry.description;
        }
    }
