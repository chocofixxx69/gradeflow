...
export async function fetchCatalogIndex(client) {
    ...
    _catalogInFlight = (async () => {
        try {
            // Shares the process-wide read of subject_catalog rather than pulling it
            // again: the data-health sweep and several analytics routes already load
            // the same table, and the column list here is a subset of SELECTS.subject_catalog.
            const rows = await readTable(client, 'subject_catalog', SELECTS.subject_catalog);
...
export function resolveSubjectCredit(catalogIndex, { scheme, branch, semester, subject_code }) {
    let s = String(scheme || '').trim();
    let b = String(branch || '').trim().toUpperCase();
    ...
    // Auto-detect scheme from VTU subject code prefix:
    // VTU 2025 NEP scheme codes start with '1B' (e.g. 1BMATE101, 1BPLC105E, 1BEE105)
    // VTU 2022 scheme codes start with 'B' (e.g. BCS301, BDS602, BAD402, BEE301)
    if (s === '2025' && code.startsWith('B') && !code.startsWith('1B')) {
        s = '2022';
    }
    ...
    // 3. Cross-branch fallback — try every canonical VTU branch.
    //    Handles shared/allied subjects (AI/DS/RI share CS core in Sem 1-4),
    //    cross-department open electives (any branch can take any dept's elective),
    //    and oddly-filed lab/audit subjects.
    const ALL_BRANCHES = ['CS', 'AI', 'DS', 'EC', 'EE', 'ME', 'CV', 'RI'];
    for (const altBranch of ALL_BRANCHES) {
        if (altBranch === b) continue; // already tried above

        const altExact = `${s}|${altBranch}|${sem}|${code}`;
        if (catalogIndex.exact.has(altExact)) {
            return { credits: catalogIndex.exact.get(altExact), source: 'cross-branch' };
        }

        // Same-family elective variant via alternate branch
        if (vm) {
            const altFamily = `${s}|${altBranch}|${sem}|${vm[1]}X`;
            if (catalogIndex.exact.has(altFamily)) {
                return { credits: catalogIndex.exact.get(altFamily), source: 'cross-branch' };
            }
        }

        // Cross-dept open elective via alternate branch dept slot
        if (dm) {
            const [, leadingDigits, , digits] = dm;
            const altGeneric = `${s}|${altBranch}|${sem}|${leadingDigits}BXX${digits}X`;
            if (catalogIndex.exact.has(altGeneric)) {
                return { credits: catalogIndex.exact.get(altGeneric), source: 'cross-branch' };
            }
        }
    }

    // 4. Semester-relaxed fallback — if catalog has this subject in a different
    //    semester for the same branch, use that credit value. This handles cases
    //    where the scraper stored the correct semester but the catalog entry uses
    //    a different semester number (rare, but observed with reval backlog rows).
    for (const altBranch of [b, ...ALL_BRANCHES]) {
        const prefix = `${s}|${altBranch}|`;
        for (const [key, credits] of catalogIndex.exact) {
            if (key.startsWith(prefix) && key.endsWith(`|${code}`)) {
                return { credits, source: 'sem-relaxed' };
            }
        }
        // Also try variant X suffix
        if (vm) {
            const xCode = vm[1] + 'X';
            for (const [key, credits] of catalogIndex.exact) {
                if (key.startsWith(prefix) && key.endsWith(`|${xCode}`)) {
                    return { credits, source: 'sem-relaxed' };
                }
            }
        }
    ...
    // 5. Prefix normalization for 2022/2025 scheme (e.g. 1BCHES102 -> BCHES102, 1BCHEE102 -> BCHEE102)
    if (code.startsWith('1B')) {
        const strippedCode = code.slice(1);
        const subRes = resolveSubjectCredit(catalogIndex, { scheme: s, branch: b, semester: sem, subject_code: strippedCode });
        if (subRes && subRes.credits !== null) {
            return { credits: subRes.credits, source: 'prefix-normalized' };
        }
    }

    // 6. Canonical VTU NEP 2022 / 2025 subject pattern rules
    // Kannada Ability / Cultural courses: 1 credit
    if (code.includes('KBK') || code.includes('KSK')) {
        return { credits: 1, source: 'kannada-rule' };
    }
    // First-year practical & core labs: 1 credit
    if (code.includes('POPL') || code.includes('EMEL') || code.includes('ECEL') || code.endsWith('L207') || code.endsWith('L107') || code === 'BRIL305' || code === 'BMEL404' || code === 'BMEL504') {
        return { credits: 1, source: 'lab-rule' };
    }
    // Computer Aided Engineering Drawing: 3 credits
    if (code.includes('CEDEC') || code.includes('CEDM') || code.includes('CEDK')) {
        return { credits: 3, source: 'caed-rule' };
    }
    // Physics / Electronics stream: 4 credits
    if (code.includes('PHEC')) {
        return { credits: 4, source: 'physics-rule' };
    }
    // Chemistry stream: 4 credits
    if (code.includes('CHEE') || code.includes('CHEM') || code.includes('CHES')) {
        return { credits: 4, source: 'chemistry-rule' };
    }
