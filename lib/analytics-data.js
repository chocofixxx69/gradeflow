...
// minutes of reuse collapses a burst of page loads onto one build while keeping the
...
//
// The constant below was left at 60_000 (one minute) instead of the twenty
// minutes described above - every admin analytics tab (and the 3 export
// formats, and faculty reports) was re-paying the multi-second warehouse
// rebuild roughly once a minute during normal use instead of once per twenty
// minutes. Corrected to match the documented design.
const _datasetCache = new Map();
const DATASET_CACHE_TTL = 20 * 60_000;
...
async function buildWarehouse(client) {
    const [students, remarks, results, subjectMarks, classes, classStudents, facultyAssignments, examSessions, facultyList, catalogIndex] = await Promise.all([
        ...
        fetchAllPaginated('faculty_subject_assignments', 'id, faculty_id, class_id, subject_code, branch, semester, scheme', client),
...
export async function loadResultAnalysisDataset(client, { role, facultyId, filters = {} } = {}) {
    ...
    if (classId) {
        // When scoped to a specific class, keep that class regardless of current semester progression
    ...
    let scopedStudents = students.filter(s => {
        ...
        if (semester) {
            // Check if student has marks for this semester or is currently studying it
            const hasSemMarks = (marksByUsn[s.usn] || []).some(m => Number(m.semester) === Number(semester));
            const isCurrentSem = String(s.semester) === String(semester);
            if (!hasSemMarks && !isCurrentSem) return false;
        }
...
//
// Matches dynamically without arbitrary constraints:
// 1. Direct class match (class_id + subject_code)
// 2. Canonical branch + semester match
// 3. Canonical branch match
// 4. Subject code match (across the institution)
export function findFacultyAssignment(assignments, { subjectCode, branch, semester, scheme, classId }) {
    if (!assignments || !assignments.length || !subjectCode) return null;
    const cleanCode = (subjectCode || '').toUpperCase().trim();
    const cleanBranch = canonicalBranchFilter(branch);

    // 1. Direct class-scoped match (highest priority for multi-class cross-branch assignments)
    if (classId) {
        const directClassMatch = assignments.find(a =>
            (a.subject_code || '').toUpperCase().trim() === cleanCode &&
            a.class_id === classId
        );
        if (directClassMatch) return directClassMatch;
    }

    // 2. Exact/canonical branch + semester match (class-scoped or class-agnostic)
    const branchSemMatches = assignments.filter(a => {
        if ((a.subject_code || '').toUpperCase().trim() !== cleanCode) return false;
        if (classId && a.class_id && a.class_id !== classId) return false;
        const aBranch = canonicalBranchFilter(a.branch);
        if (cleanBranch && aBranch && aBranch !== cleanBranch) return false;
        if (semester && a.semester && String(a.semester) !== String(semester)) return false;
        if (scheme && a.scheme && a.scheme !== scheme) return false;
        return true;
    });

    if (branchSemMatches.length) {
        return (classId ? branchSemMatches.find(a => a.class_id === classId) : null)
            || branchSemMatches.find(a => !a.class_id)
            || branchSemMatches[0];
    }

    // 3. Subject + Canonical Branch match (semester-agnostic fallback)
    const branchMatches = assignments.filter(a => {
        if ((a.subject_code || '').toUpperCase().trim() !== cleanCode) return false;
        if (classId && a.class_id && a.class_id !== classId) return false;
        const aBranch = canonicalBranchFilter(a.branch);
        if (cleanBranch && aBranch && aBranch !== cleanBranch) return false;
    ...
    if (branchMatches.length) {
        return (classId ? branchMatches.find(a => a.class_id === classId) : null)
            || branchMatches.find(a => !a.class_id)
            || branchMatches[0];
    }

    // 4. Subject Code Match (dynamic fallback across entire database)
    // If the subject has been assigned to a particular faculty, replicate it dynamically
    const subjectMatches = assignments.filter(a =>
        (a.subject_code || '').toUpperCase().trim() === cleanCode
    );

    if (subjectMatches.length) {
        return (classId ? subjectMatches.find(a => a.class_id === classId) : null)
            || subjectMatches.find(a => !a.class_id)
            || subjectMatches[0];
    }
