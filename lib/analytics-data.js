...
export async function fetchDynamicStudents(supabaseAdmin, { branch = '', select = 'id, usn, name, branch, semester, year, lateral_entry' } = {}) {
    ...
    while (true) {
        ...
        if (branch && branch !== 'ALL') {
            ...
            if (b === 'AI' || b === 'AIML' || b === 'CI') {
                q = q.or('branch.ilike.%AI%,branch.ilike.%CI%,branch.ilike.%AIML%');
            } else if (b === 'DS' || b === 'CD') {
                q = q.or('branch.ilike.%DS%,branch.ilike.%CD%,branch.ilike.%DATA%');
            } else if (b === 'CS' || b === 'CSE') {
                q = q.or('branch.ilike.%CS%,branch.ilike.%COMPUTER%');
            } else {
