...
import { calculateAcademicRecord, normalizeBranch } from './vtuAcademicEngine';
import { fetchCatalogIndex, resolveSubjectCredit } from './subjectCreditResolver';
...
export async function loadResultAnalysisDataset(client, { role, facultyId, filters = {} } = {}) {
    const [students, remarks, results, subjectMarks, classes, classStudents, facultyAssignments, examSessions, facultyList, catalogIndex] = await Promise.all([
        ...
        fetchAllPaginated('classes', 'id, name, branch, semester, section, subject_name, subject_code, faculty_id, academic_year, created_at', client, 'created_at', false),
        fetchAllPaginated('class_students', 'class_id, usn', client),
        fetchAllPaginated('faculty_subject_assignments', 'id, faculty_id, class_id, subject_code, subject_name', client),
        fetchAllPaginated('exam_sessions', 'id, name, start_date, end_date, status, academic_year', client),
