...
import { classify } from './vtuGrades';
...
export function getAdminClient() {
    ...
    if (!serviceKey && !warnedAnonFallback) {
        ...
        console.warn('[analytics-data] SUPABASE_SERVICE_KEY not set — falling back to anon key for admin analytics client.');
    }
    return createClient(
...
// Canonical backlog derivation. Source of truth is subject_marks.is_backlog —
// NOT a stored counter column (academic_remarks has no backlog_count/is_all_clear
// column; storing a derived count redundantly risks drift from the real marks).
export function computeBacklogs(subjectMarksForUsn) {
    ...
    for (const m of subjectMarksForUsn) {
        ...
        const isFail = m.is_backlog === true || m.is_backlog === 'true' || g === 'F' || g === 'A' || g === 'FAIL' || g === 'ABSENT' || g === 'NP' || g === 'NE' || g === 'X' || (ext > 0 && ext < 18) || (tot > 0 && tot < 40) || resStr.includes('F');
        
        if (!isFail) continue;
        bySemester[m.semester] = (bySemester[m.semester] || 0) + 1;
        failedSubjects.push({
...
export async function loadResultAnalysisDataset(client, { role, facultyId, filters = {} } = {}) {
    const [students, remarks, results, subjectMarks, classes, classStudents, facultyAssignments, examSessions, facultyList, subjectCatalog] = await Promise.all([
        ...
        fetchAllPaginated('exam_sessions', '*', client),
        fetchAllPaginated('faculty_onboarding', 'id, full_name, department', client),
        fetchAllPaginated('subjects', 'code, name, credits, semester, branch, scheme', client),
    ]);

    const facultyById = {};
    for (const f of facultyList) facultyById[f.id] = f;

    // Subject catalog is the canonical name/credits source when populated —
    // key by code+branch+semester+scheme first, fall back to code-only.
    const subjectCatalogByKey = {};
    const subjectCatalogByCode = {};
    for (const sub of subjectCatalog) {
        subjectCatalogByKey[`${sub.code}|${sub.branch}|${sub.semester}|${sub.scheme}`] = sub;
        if (!subjectCatalogByCode[sub.code]) subjectCatalogByCode[sub.code] = sub;
    }
    function lookupSubjectCatalog({ code, branch, semester, scheme }) {
        return subjectCatalogByKey[`${code}|${branch}|${semester}|${scheme}`] || subjectCatalogByCode[code] || null;
    }
...
export function buildStudentRow(s, dataset) {
    ...
    const cgpa = weightedCGPA(rem, dataset.creditsByUsnSem[s.usn] || {});
    const marks = dataset.marksByUsn[s.usn] || [];
    const { totalBacklogs, maxSemesterBacklogs, failedSubjects, isAllClear } = computeBacklogs(marks);
    const totalCredits = Object.values(dataset.creditsByUsnSem[s.usn] || {}).reduce((a, b) => a + b, 0);
    const earnedCredits = marks.filter(m => m.passed).reduce((a, m) => a + (m.credits || 0), 0);
    ...
    // "Current" SGPA = the remark for the highest semester on record, not
    // whatever row Postgres happens to return last (fetch order is not
    // guaranteed to be semester-ordered).
    const latestRemark = rem.length
        ? rem.reduce((latest, r) => (r.semester ?? -1) > (latest.semester ?? -1) ? r : latest)
        : null;
    ...
    return {
        ...
        sgpa: latestRemark ? latestRemark.sgpa : null,
        cgpa,
