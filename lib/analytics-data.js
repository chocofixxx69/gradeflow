...
export async function loadResultAnalysisDataset(client, { role, facultyId, filters = {} } = {}) {
    const [students, remarks, results, subjectMarks, classes, classStudents, facultyAssignments, examSessions, facultyList] = await Promise.all([
        fetchAllPaginated('students', 'usn, name, branch, semester, scheme, lateral_entry', client),
        fetchAllPaginated('academic_remarks', 'student_usn, semester, sgpa', client),
...
export function buildStudentRow(s, dataset) {
    ...
    return {
        ...
        sgpa: rem.length ? rem[rem.length - 1].sgpa : null,
        cgpa,
